...
Thanks for considering a contribution. Riffle stays small on purpose — please open an issue before starting on anything beyond a bug fix, so we can agree on the approach first.
...
RIFFLE_E2E_BACKGROUND=1 pnpm run test:electron
```
...
- **The Vault Engine owns filesystem and collection work.** Renderer code consumes domain-shaped services over the typed `window.riffle` preload bridge; it never imports Node or Electron.
...
Open an issue with your operating system, Riffle version, and reproduction steps. If it is a vault or data issue, mention whether it reproduces with a fresh vault folder.
