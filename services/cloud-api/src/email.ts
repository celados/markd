...
const RIFFLE_DOWNLOAD_URL = "https://usemarkd.app/download";
const RIFFLE_LOGO_URL = "https://usemarkd.app/apple-icon.png";
...
export async function sendOtpEmail(
  ...
  await env.EMAIL.send({
    to: email,
    from: { email: OTP_SENDER, name: "Riffle" },
    subject: "Your Riffle sign-in code",
    text: [
      "Sign in to Riffle",
      "",
  ...
  <body style="margin:0;background:#f5f5f3;color:#171717;font-family:Arial,sans-serif">
    <div style="max-width:480px;margin:0 auto;padding:48px 20px">
      <div style="background:#ffffff;border:1px solid #e5e5e2;border-radius:16px;padding:32px">
        <p style="margin:0 0 24px;font-size:12px;letter-spacing:.14em;text-transform:uppercase;color:#737373">Riffle</p>
        <h1 style="margin:0;font-size:22px;line-height:1.25">Sign in to Riffle</h1>
        <p style="margin:12px 0 0;font-size:14px;line-height:1.6;color:#666">Enter this code in the Riffle desktop app:</p>
...
export async function sendWelcomeEmail(
  ...
  await env.EMAIL.send({
    to: email,
    from: { email: OTP_SENDER, name: "saurabh from riffle" },
    subject: "welcome to riffle",
    text: [
      `hey ${name},`,
      "",
      "i'm saurabh. thanks for trying riffle.",
      "",
      "riffle doesn't want to become your second brain. your first one already has enough going on.",
      "",
  ...
  <body class="email-bg" style="margin:0;background:#ffffff;font-family:Arial,sans-serif;-webkit-font-smoothing:antialiased">
    <div class="email-bg" style="min-height:100%;background:#ffffff;padding:56px 24px">
      <div style="max-width:520px;margin:0 auto">
        <img src="${RIFFLE_LOGO_URL}" width="44" height="44" alt="Riffle" style="display:block;width:44px;height:44px;border:0;border-radius:11px">
        <h1 class="email-text" style="margin:36px 0 0;color:#171717;font-size:24px;line-height:1.15;letter-spacing:-.02em">hey ${name},</h1>
        <p class="email-muted" style="margin:20px 0 0;color:#5f5f5f;font-size:16px;line-height:1.6">i'm saurabh. thanks for trying riffle.</p>
        <p class="email-muted" style="margin:16px 0 0;color:#5f5f5f;font-size:16px;line-height:1.6">riffle doesn't want to become your second brain. your first one already has enough going on.</p>
        ...
        <p class="email-muted" style="margin:16px 0 0;color:#5f5f5f;font-size:16px;line-height:1.6">if you haven't downloaded the app yet, <a class="email-link" href="${RIFFLE_DOWNLOAD_URL}" style="color:#171717;text-decoration:underline;text-decoration-thickness:1px;text-underline-offset:3px">you can find it here</a>.</p>
